...
screen navigation():
    ...
    vbox:
        ...
        if _in_replay:
            ...
            textbutton _("回主畫面") action MainMenu()
...
screen main_menu():
    ...
    # 播放主選單音樂
    on "show" action Play("music", "audio/background/大廳.mp3", loop=True)
...
screen hp_display() :
    frame:
        ...
        padding (10, 10, 10, 10)
        background "#5b4c4250"
        text "精神力: [hp]" color "#ffffff" size 32


screen note_screen(text):
    modal True  # 鎖定畫面，直到關閉為止
    ...
    frame:
        align (0.5, 0.5)   # 置中
        padding (10, 10)
        background Solid("#610c0c9d") # ← 你可以換自己的筆記圖片
        xsize 1000
        ysize 900
...
screen quiz_screen(question, answer_list):
    # 參數說明：
    # question: 問題文字
    # answer_list: 選項列表 [("文字", 正確與否), ...]

    default shaking_idx = -1      # 記錄哪個按鈕在晃
    default status_msg = ""       # 錯誤訊息
    ...
    vbox:
        ...
        text question:
            ...
            size 60               # 字體大小
            color "#ffffff"       # 純白
            outlines [(3, "#000000", 0, 0)] # 加粗黑邊，讓字更清楚
        ...
        vbox:
            ...
            for idx, (option_text, is_correct) in enumerate(answer_list):
                
                button:
                    # 設定按鈕大小與對齊
                    xalign 0.5
                    xsize 600           # 固定寬度，看起來比較整齊
                    padding (20, 20)    # 內距

                    # --- 紅底框框設定 ---
                    background Solid("#782c2c")       # 平常：深紅色
                    hover_background Solid("#ae7979") # 滑鼠碰到：亮紅色
                    ...
                    if is_correct:
                        ...
                        # 選錯：晃動 + 扣血 + 檢查死亡
                        action [
                            SetScreenVariable("shaking_idx", idx),
                            SetVariable("hp", hp - 15),
                            SetScreenVariable("status_msg", "答案不對...精神力 -15"),
                            If(hp <= 15, Return("game_over")) 
                        ]
        ...
        if status_msg:
            text status_msg:
                ...
                at transform:
                    alpha 0.0
                    linear 0.2 alpha 1.0 # 淡入效果

    if shaking_idx != -1:
            # 0.3秒後，把 shaking_idx 設回 -1 (重置狀態)
            timer 0.3 action SetScreenVariable("shaking_idx", -1)
